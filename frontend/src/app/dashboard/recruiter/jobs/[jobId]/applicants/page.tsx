...
const mockApplicants = [
  { id: 1, name: "Alice Smith", email: "alice.s@example.com", dateApplied: "2026-06-20", status: "Applied" },
  { id: 2, name: "Bob Johnson", email: "bob.j@example.com", dateApplied: "2026-06-21", status: "Under Review" },
  { id: 3, name: "Charlie Davis", email: "charlie.d@example.com", dateApplied: "2026-06-22", status: "Shortlisted" },
];
...
export default function JobApplicantsPage() {
  ...
  const handleStatusChange = (id: number, newStatus: string) => {
    ...
    const applicantName = applicants.find(app => app.id === id)?.name;
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody className="divide-y divide-gray-100">
              {applicants.map((applicant) => (
                <tr key={applicant.id} className="hover:bg-indigo-50/30 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">
                    {new Date(applicant.dateApplied).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 w-48">
                    <div className="relative">
                      <select
                        ...
                        className="block w-full appearance-none bg-white border border-gray-200 text-gray-700 py-2 px-4 pr-8 rounded-lg leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm font-medium transition-all hover:border-gray-300 cursor-pointer shadow-sm"
                      >
                        {statuses.map(status => (
